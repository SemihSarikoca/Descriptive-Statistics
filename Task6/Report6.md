...
sns.scatterplot(data=df_estate, x='n_convenience', y='price_twd_msq', alpha=0.7)
plt.title('Number of Convenience Stores vs. House Price per m²')
plt.xlabel('Number of Convenience Stores')
plt.ylabel('Price (TWD/m²)')
plt.show()

# Pearson correlation
corr_nc = df_estate['n_convenience'].corr(df_estate['price_twd_msq'])
print(f"Correlation (n_convenience vs. price_twd_msq): {corr_nc:.2f}")
```
...
![png](Exercise10_files/Exercise10_17_0.png)
    ...
    Correlation (n_convenience vs. price_twd_msq): 0.57
...
plt.title('House Age vs. House Price per m²')
plt.xlabel('House Age (years)')
plt.ylabel('Price (TWD/m²)')
plt.show()

corr_age = df_estate['house_age_years'].corr(df_estate['price_twd_msq'])
print(f"Correlation (house_age_years vs. price_twd_msq): {corr_age:.2f}")
```
...
![png](Exercise10_files/Exercise10_19_0.png)
    ...
    Correlation (house_age_years vs. price_twd_msq): -0.21
...
```python
plt.figure(figsize=(7, 5))
sns.scatterplot(data=df_estate, x='dist_to_mrt_m', y='price_twd_msq', alpha=0.7)
plt.title('Distance to MRT vs. House Price per m²')
plt.xlabel('Distance to MRT (m)')
plt.ylabel('Price (TWD/m²)')
plt.show()

corr_mrt = df_estate['dist_to_mrt_m'].corr(df_estate['price_twd_msq'])
print(f"Correlation (dist_to_mrt_m vs. price_twd_msq): {corr_mrt:.2f}")
```
...
![png](Exercise10_files/Exercise10_21_0.png)
    ...
    Correlation (dist_to_mrt_m vs. price_twd_msq): -0.67
...
```python
g = sns.FacetGrid(df_estate, col='house_age_cat_str', height=4, aspect=1)
g.map(plt.hist, 'price_twd_msq', bins=10, color='skyblue', edgecolor='black')
g.set_axis_labels("Price (TWD/m²)", "Count")
g.fig.suptitle('Distribution of House Price per m² by Age Group', y=1.05)
plt.show()
```
...
```python
summary = df_estate.groupby('house_age_cat_str')['price_twd_msq'].agg(['mean', 'std', 'median', 'min', 'max', 'count'])
print(summary)
```
    ...
    /var/folders/y1/k00y87l50_59zy_2c9drqj000000gn/T/ipykernel_26693/1386233854.py:1: FutureWarning: The default of observed=False is deprecated and will be changed to True in a future version of pandas. Pass observed=False to retain current behavior or observed=True to adopt the future default and silence this warning.
      summary = df_estate.groupby('house_age_cat_str')['price_twd_msq'].agg(['mean', 'std', 'median', 'min', 'max', 'count'])
...
```
    ...
    Date:                Fri, 30 May 2025   Prob (F-statistic):           4.64e-56
    Time:                        21:04:45   Log-Likelihood:                -1542.5
...
<tr>
  <th>Date:</th>             <td>Fri, 30 May 2025</td> <th>  Prob (F-statistic):</th> <td>7.84e-59</td>
</tr>
<tr>
  <th>Time:</th>                 <td>21:04:45</td>     <th>  Log-Likelihood:    </th> <td> -1530.2</td>
</tr>
<tr>
...
```python
plt.figure(figsize=(7, 5))
sns.scatterplot(x=df_estate['dist_to_mrt_m'], y=df_estate['price_twd_msq'], alpha=0.7)
plt.title('Distance to MRT vs. House Price per m²')
plt.xlabel('Distance to MRT (m)')
plt.ylabel('Price (TWD/m²)')
plt.show()
...
plt.figure(figsize=(7, 5))
sns.scatterplot(x=np.log1p(df_estate['dist_to_mrt_m']), y=df_estate['price_twd_msq'], alpha=0.7)
plt.title('Log(Distance to MRT) vs. House Price per m²')
...
# Add log-transformed distance to the dataframe
df_estate['log_dist_to_mrt'] = np.log1p(df_estate['dist_to_mrt_m'])

# Estimate Model 3 with the transformed distance
...
```
    ...
    Date:                Fri, 30 May 2025   Prob (F-statistic):           6.31e-73
    Time:                        21:04:45   Log-Likelihood:                -1497.3
...
# Remove insignificant variable(s), e.g., house_age_30_45
...
```
    ...
    Date:                Fri, 30 May 2025   Prob (F-statistic):           4.99e-74
    Time:                        21:04:46   Log-Likelihood:                -1497.7
    ...
    log_dist_to_mrt    -8.6754      0.404    -21.492      0.000      -9.469      -7.882
...
```
    ...
    Date:                Fri, 30 May 2025   Prob (F-statistic):           5.61e-62
    Time:                        21:04:46   Log-Likelihood:                -1525.6
...
```python
fig = plt.figure(figsize=(12, 10))
sm.graphics.plot_regress_exog(model_forward, features_forward[0], fig=fig)
plt.show()
