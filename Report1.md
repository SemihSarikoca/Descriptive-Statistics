...
```python
users.age.value_counts(bins=10).idxmin()
```




    Interval(6.933, 13.6, closed='right')
...
```python
disipline = euro12[['Team', 'Yellow Cards', 'Red Cards']]
disipline
```
...
```python
disipline.sort_values(['Red Cards', 'Yellow Cards'])
```
...
```python
disipline.groupby('Team')['Yellow Cards'].mean()
```
...
```python
drinks.groupby('continent').total_litres_of_pure_alcohol.aggregate(['mean'])
```
...
```python
drinks.groupby('continent').total_litres_of_pure_alcohol.aggregate(['median'])
```
